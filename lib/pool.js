...
const { ConnectionError, WaitTimeoutError } = require("./errors");
...
class RedisConnectionPool extends Logger {
  ...
  constructor(config) {
    ...
    const defaultConfig = { connectionLimit: 10, waitTimeout: 5000 };
   ...
   */
  waitForRelease() {
    const timeout = this.config.waitTimeout;

    return new Promise((res, rej) => {
      setTimeout(() => rej(new WaitTimeoutError()), timeout);
      this.queue.push(res);
    });
  }
