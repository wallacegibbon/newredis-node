...
class RedisConnectionPool extends Logger {
  ...
  showConnectionsDetail() {
    this.debug(`B&F: ${this.connections.map(this.formatConnInfo)}`);
    this.debug(`Size&Broken: ${this.poolSize}, ${this.brokenCnt}`);
  }
...
class PoolConnection extends Logger {
  ...
  async transaction(commands) {
    this.trace(`Starting a redis transaction: ${inspect(commands)}`);
    try {
