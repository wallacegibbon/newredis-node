...
class RedisConnectionPool extends Logger {
  ...
  /**
   * Show connection pool status. (`broken` and `flag` of each connection)
   */
  showPoolStatus() {
    this.info(`Pool status: ${this.connections.map(this.formatConnInfo)}`);
  }


  /**
   * Format the `broken` and `free` flag of a PoolConnection.
   */
  formatConnInfo(conn) {
    return `<broken:${conn.broken},free:${conn.free}>`;
  }
