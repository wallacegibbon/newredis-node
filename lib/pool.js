...
class RedisPool {
  ...
  constructor(config) {
    ...
    emitter.on("release", this.waitQueueHandleNormal.bind(this));
    emitter.on("error", this.waitQueueHandleError.bind(this));
   ...
   */
  waitQueueHandleNormal(idx) {
    logger.debug(`Redis Connection(pool idx ${idx}) released.`);
  ...
  /**
   * When redis server went error, the wait queue should be emptied. Those
   * who wait for connection will get an error.
   */
  waitQueueHandleError(err) {
  ...
  async getNewConnection() {
    logger.debug("Trying to get a new Connection.");
    ...
    logger.debug("New connection got.");
    return conn;
  }
  ...
  async getOldConnection() {
    logger.debug("Trying to get a old Connection.");
    ...
    logger.debug(`Old connection got(pool idx ${conn.idx}).`);
    return conn;
  }
...
class PoolConnection {
   ...
   */
  release() {
    logger.debug(`Trying to release connection(pool idx ${this.idx})`);
    this.free = true;
