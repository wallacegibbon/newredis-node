...
class RedisConnectionPool extends Logger {
  ...
  constructor(config) {
    ...
    Object.assign(this, { queue: [], connections: [], size: 0, brokenCnt: 0 });
  ...
  /**
   * When size and brokenCnt(the broken TCP connection counter) equals,
   * there is no live TCP connection anymore.
   */
  noLiveTCPconnection() {
    return this.size != 0 && this.size === this.brokenCnt;
  }
  ...
  showConnectionsDetail() {
    this.info(`B&F: ${this.connections.map(this.formatConnInfo)}`);
    this.info(`Size&Broken: ${this.size}, ${this.brokenCnt}`);
  }
...
class PoolConnection extends Logger {
   ...
   */
  async initialize() {
    this.info(`Initializing a PoolConnection object(pool idx ${this.idx})`);
