...
function encodeRedisCommand(commands) {
  const result = commands.map(adjustCommandArgument)
                  .map(x => Buffer.from(`\$${x.length}\r\n${x}\r\n`));

  result.unshift(Buffer.from(`*${commands.length}\r\n`));
  const size = result.map(b => b.length).reduce((a, b) => a + b);
...
function adjustCommandArgument(argObj) {
  if (typeof argObj === "number") {
    ...
    throw new Error(`Invalid redis command argument: ${argObj}`);
  }
}
